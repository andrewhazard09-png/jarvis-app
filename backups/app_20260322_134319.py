...
def chat():
    ...
    # Screen reading
    screen_triggers = ['look at my screen', 'what do you see', 'whats on my screen', 'read my screen', 'can you see', 'what am i looking at', 'help me with this']
    ...
    no_search = ['hello', 'hi', 'hey', 'thanks', 'thank you', 'who are you', 'what are you', 'modify yourself', 'rewrite', 'open', 'list files', 'show files']
    needs_search = not any(t in msg_lower for t in no_search) and '?' in message or any(w in msg_lower for w in ['what', 'who', 'when', 'where', 'how', 'whats', 'latest', 'news', 'tell me about'])
    if needs_search and not any(w in msg_lower for w in ['weather', 'temperature', 'news', 'headlines']):
        search_results = web_search(message)
        def generate():
            prompt = 'Based on these real search results, answer this question: ' + message + ' Results: ' + search_results + ' Be concise and accurate. Never make up data not in the results.'
            groq_res = requests.post('https://api.groq.com/openai/v1/chat/completions',
                headers={'Authorization': 'Bearer ' + GROQ_KEY, 'Content-Type': 'application/json'},
                json={'model': 'llama-3.1-8b-instant', 'messages': [{'role': 'user', 'content': prompt}], 'stream': True, 'max_tokens': 512},
    ...
    # Wire in knowledge context
    try:
        memory_context = memory_context + knowledge_context
    except:
        pass
