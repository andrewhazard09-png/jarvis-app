...
IMPORTANT: You can modify your own code ONLY when the user explicitly says words like "add a feature", "change the UI", "modify yourself", "rewrite", or "update your code". For ALL other messages including greetings, questions, and tasks, respond normally in plain text. When and ONLY when explicitly asked to modify code, respond with this exact format:

FILE: index.html
CODE:
(the complete new index.html file contents here)
END

CRITICAL RULES:
- The filename MUST be exactly "index.html" or "app.py" — nothing else
- Never invent filenames like src/sidebar.py or components/anything
- Always write the COMPLETE file contents, not just the changed part
- Do not add any explanation before or after the FILE/CODE/END block"""
...
def chat():
    ...
    def generate():
        yield f"[ROUTING TO: {selected.upper()}]\n\n"
        try:
