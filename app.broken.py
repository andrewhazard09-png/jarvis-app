...
def chat():
    ...
    # Smart search detection - ask Groq if it needs current info
        ...
        try:
            res = requests.post('https://api.groq.com/openai/v1/chat/completions',
                ...
                json={
                    ...
                    'messages': [{'role': 'user', 'content': f'Does answering this question require current real-time information, live scores, recent news, or anything that could have changed in the last week? Answer ONLY yes or no. Question: {msg}'}],
                    'max_tokens': 5
                },
                timeout=10
            )
    ...
    # Smart search - use AI to decide if current info is needed
    needs_search = needs_current_info(message)

    # Sports scores via ESPN - only for direct score requests
    ...
    # Agent mode - only for explicit computer tasks
...
def check_news():
    while True:
        try:
            res = requests.get('https://newsapi.org/v2/everything?q=sports&sortBy=publishedAt&pageSize=3&apiKey=' + NEWS_KEY, timeout=10)
            articles = res.json().get('articles', [])
            if articles:
                with open(os.path.expanduser('~/jarvis-app/alerts.json'), 'r+') as f:
                    alerts = json.load(f)
                    alerts.append({'type': 'news', 'message': articles[0]['title']})
                    f.seek(0)
...
def check_battery():
    while True:
        try:
            battery_level = subprocess.check_output(['pmset', '-g', 'batt']).decode('utf-8').split('\n')[0].split(':')[1].strip()
            if int(battery_level.split('%')[0]) < 20:
                with open(os.path.expanduser('~/jarvis-app/alerts.json'), 'r+') as f:
                    alerts = json.load(f)
                    alerts.append({'type': 'battery', 'message': 'Low battery level'})
                    f.seek(0)
                    json.dump(alerts, f, indent=2)
        except:
            pass
...
def clear_history():
    try:
        with open(os.path.expanduser('~/jarvis-app/chat_history.json'), 'w') as f:
...
def memory():
    try:
        with open(os.path.expanduser('~/jarvis-app/memory.json'), 'r') as f:
...
def system_stats():
    try:
        cpu_usage = subprocess.check_output(['top', '-l', '1', '-s', '0']).decode('utf-8').split('\n')[0].split(':')[1].strip()
        ram_usage = subprocess.check_output(['vm_stat']).decode('utf-8').split('\n')[0].split(':')[1].strip()
        battery_level = subprocess.check_output(['pmset', '-g', 'batt']).decode('utf-8').split('\n')[0].split(':')[1].strip()
        disk_space = subprocess.check_output(['df', '-h']).decode('utf-8').split('\n')[0].split(':')[1].strip()
        return jsonify({'cpu_usage': cpu_usage, 'ram_usage': ram_usage, 'battery_level': battery_level, 'disk_space': disk_space})
    except:
        return jsonify({'error': 'Failed to retrieve system stats'})
...
def feedback():
    ...
    try:
        with open(os.path.expanduser('~/jarvis-app/feedback.log'), 'a') as f:
            f.write(json.dumps({'rating': rating, 'message': message[:100], 'topic': topic, 'date': datetime.datetime.now().isoformat()}) + '\n')
    except:
        pass
    return jsonify({'status': 'ok'})
...
def personality():
    ...
    try:
        with open(os.path.expanduser('~/jarvis-app/personality.json'), 'w') as f:
            json.dump({'name': name, 'tone': tone, 'traits': traits}, f, indent=2)
    except:
        pass
    return jsonify({'status': 'ok'})
...
def chat():
    ...
    def generate():
        try:
            ...
            # Log conversation
            with open(os.path.expanduser('~/jarvis-app/conversations.log'), 'a') as f:
                f.write(f"[{datetime.datetime.now().isoformat()}] JARVIS: {delta}\n")
        except Exception as e:
            # Fallback to OpenRouter
            try:
                or_res = requests.post('https://openrouter.ai/api/v1/chat/completions',
